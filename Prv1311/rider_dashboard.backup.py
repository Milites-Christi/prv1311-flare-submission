...
def _link_row(links: dict) -> str:
    ...
    def add(label, url):
        if url:
            out.append(f'<a href="{url}" target="_blank" style="color:{BLUE};'
...
def _render_profile(symbol: str):
    ...
    cats = [c for c in (d.get("categories") or []) if c][:6]
    ...
    gecko_url = f"https://www.coingecko.com/en/coins/{coin_id}"

    st.sidebar.markdown(
        f"""<div style="margin:6px 0 8px 0;">
        <span style="color:{BLUE};font-size:18px;font-weight:700;">{name}</span>
        <span style="color:{SLATE};font-size:13px;margin-left:6px;">{symbol}</span>
        </div>""",
        unsafe_allow_html=True,
    )
    ...
    if up_pct:
        signal.append(f"{up_pct:.0f}% bullish sentiment")
...
def render_sidebar():
    ...
    if pinned:
        ...
        for s in pinned:
            if st.sidebar.button(f"● {s}", key=f"td_{s}"):
    ...
    for s in rest:
        if st.sidebar.button(s, key=f"td_{s}"):
            st.session_state["td_selected"] = s
            st.rerun()